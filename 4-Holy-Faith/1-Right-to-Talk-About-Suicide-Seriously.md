...
>     - Convolution is another method in deep learning, [from which MHDPA can be composed](https://github.com/slerman12/BrokenWisdoms/blob/Ancillary/Conv-is-all-you-need.md)—via three convolutions (and by the way, of arbitrary kernel size, stride, padding, etc.).
>     - So as it happens, there is a more fundamental method, and the former (attention) is not "all you need."
