...
>     - Convolution is another method in deep learning, [from which MHDPA can be composed](https://github.com/slerman12/BrokenWisdoms/blob/Ancillary/Conv-is-all-you-need.md)—via three convolutions (and by the way, of arbitrary kernel size, stride, padding, etc.—not sure if I can take credit inside of a parenthetical, but this was/is my novel generalization, especially in the context of not needing image patches in ViTs, though the use of conv layers for implementation is not novel).
